...
parsed_address_list = []
account_list = []
col_headers = [idHeader]

# All column headers, the above col_headers is to pull only the existing address part types
#
# column_headers = [
#     'AddressNumberPrefix',
#     'AddressNumber',
...
# Static file path %USERPROFILE%/Documents/scripts/Parser/addresses.xlsx"
wb_dir = str(
    os.path.join(
        os.environ["USERPROFILE"], "Documents", "scripts", "Parser", "addresses.xlsx"
    )
)

# Manual Sheet Name entry commented out for now
#
...
dirtyDF = pd.read_excel(wb_dir)


# Get name of input column headers
idHeader = str(dirtyDF.index(0))
addrHeader = str(dirtyDF.index(1))

# Define error function
def if_tag_error(parsed_string, i):
    errored_addresses_dict[str(dirtyDF[idHeader][i])] = parsed_string


# Read parsed addresses into dictionary
for i in dirtyDF.index:

    # Error exception
    try:
        address_dict[str(dirtyDF[idHeader][i])] = usaddress.tag(
            str(dirtyDF[addrHeader][i])
        )
        account_list.append(dirtyDF[idHeader][i])
...
parsedDF = pd.DataFrame.from_dict(address_dict, orient="index")
...
parsedDF.to_excel(writer, "output", index=True)
writer.save()
...
output_wb = load_workbook(
    filename=str(
        os.path.join(
            os.environ["USERPROFILE"], "Documents", "scripts", "Parser", "output.xlsx"
        )
    )
)


# Create data sheets for parsed and errored addresses, writing the column headers.
parsed_addr_sheet = output_wb.create_sheet("parsed")
parsed_addr_sheet.append(defineColumns(address_dict))

error_worksheet = output_wb.create_sheet("errors")
error_worksheet.append(defineColumns(errored_address_dict))

# Function to grab address part types
def defineColumns(parsedDict):

    # Temp list to store address part types, this will be returned as a list
    tempHeaderList = []

    # Look in each individual address dictionary
    for oDict in parsedDict:

        # Look at each part of each address
        for addrPart in oDict:

            # If address part doesn't exist, append it to the temporary list
            if addrPart[0] not in tempHeaderList:
                tempHeaderList.append(addrPart[0])
    return tempHeaderList


# Commented out creation of all possible header types to test new method for retrieving headers
#
# Create parsed and split up address sheet headers
# parsed_addr_sheet['A1'] = str('Unique Identifier')
...
# error_worksheet['A1'] = str('Unique Identifier')
...
for key, value in err_address.items():
    ...
    [
        (matchkey,) + tuple(elem for _, elem in group)
        for matchkey, group in groupby(err_address[key], lambda pair: pair[0])
    ]
    ...
    for i in range(len(err_address[key])):
        ...
        # Set value of cell
        error_worksheet.cell(row=row_num, column=col).value = str(
            err_address[key][i][1]
        )
